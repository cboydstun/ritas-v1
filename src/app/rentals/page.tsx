...
export default function RentalsPage() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-light via-margarita/10 to-teal/20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
        <div className="grid lg:grid-cols-2 gap-8">
          {machinePackages.map((machine) => (
            <div
              key={machine.type}
              className="relative bg-white rounded-2xl shadow-xl overflow-hidden group hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1"
            >
              <div className="p-8">
                <h3 className="text-2xl font-bold text-charcoal mb-2">
                  {machine.name}
                </h3>
                <p className="text-charcoal/70 mb-6">{machine.description}</p>
                ...
                <div className="space-y-4 mb-8">
                  ...
                  {machine.mixerOptions.map((option) => (
                    <div
                      key={option.type}
                      className="flex items-center justify-between p-4 rounded-lg bg-light hover:bg-margarita/10 transition-colors"
                    >
                      <div>
      ...
      {/* FAQ Section */}
      <div className="relative bg-white/80 backdrop-blur-lg">
