...
export class QuickBooksService {
  ...
  public async findOrCreateCustomer(customerData: OrderFormData['customer']): Promise<string> {
    try {
  ...
  public async findOrCreateItem(name: string, description: string, unitPrice: number): Promise<string> {
    try {
      await this.initialize();
  ...
  public async createInvoice(rental: any): Promise<QBInvoice> {
    try {
      await this.initialize();
...
export async function generateQuickBooksInvoice(rental: any): Promise<QBInvoice> {
  try {
...
export async function logQuickBooksError(rentalId: string, error: any): Promise<void> {
  try {
    await Rental.findByIdAndUpdate(rentalId, {
      $set: {
        'quickbooks.syncStatus': 'failed',
        'quickbooks.lastSyncAttempt': new Date(),
        'quickbooks.syncError': error.message || JSON.stringify(error)
      }
    });
  } catch (dbError) {
