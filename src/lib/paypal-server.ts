  ...
  try {
    ...
    if (sdkModule.default) {
      ...
      if (sdkModule.default.core) {
        console.log(
          "PayPal SDK default.core keys:",
          Object.keys(sdkModule.default.core),
        );
      }
    }
      ...
      execute: async (request: any) => {
        try {
          ...
          if (!isProd) {
            ...
            console.log(
              `Processing request with path: ${request.path} and verb: ${request.verb}`,
            );
            ...
            if (request.path.includes("/capture") && request.verb === "POST") {
              ...
              // Add debug logging
              console.log(
                `Sending capture request for order ${orderId} to ${url}`,
              );
            }
            ...
            if (!response.ok) {
              ...
              console.error(
                `PayPal API Error Response: ${JSON.stringify(errorData, null, 2)}`,
              );
            ...
            console.log(
              `PayPal API Success Response: ${JSON.stringify(
                {
                  id: responseData.id,
    ...
    throw new Error(
      `Failed to initialize PayPal SDK: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
};
