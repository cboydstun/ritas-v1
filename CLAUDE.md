...
Model tests are offline `doc.validate()` with one deliberate exception: `src/models/__tests__/deliveryZonesPersist.test.ts` spins up `mongodb-memory-server` and reads back through the **raw driver**. It has to. Mongoose applies schema defaults on _hydration_, so a document holding no `deliveryZones` reads back through the model as a complete one — every predicate that asks the hydrated document answers "already there" while the stored document holds nothing. bounce-v3 shipped a self-migration built on that assumption; it did nothing, passed its tests, and production ran with an empty field while the API served defaults. The raw collection is the only place that difference is visible. `beforeAll` carries a 120s timeout because the first run on a machine (CI included) downloads a mongod binary. Do not widen the exception: every other model test stays offline.
...
Consent Mode v2 defaults to `granted` (inlined in `GoogleAnalytics.tsx` ahead of `config`), with `CookieConsent.tsx` offering an opt-out stored under `satx-ritas-consent`. Keep any new Ads tag consent-gated on `ad_storage`, `ad_user_data` and `ad_personalization` — `notSet` is the GTM default, so this is a thing you have to remember to do, not a thing that happens. Texas TDPSA is an opt-out regime; serving EU traffic would mean flipping those defaults to `denied`.

**The per-tag consent gating is written but not live, and this file previously claimed otherwise.** The `consentStatus: needed` settings on the Conversion Linker and the purchase tag exist only in GTM **workspace 9, which has never been published** — the session that made them on 2026-08-12 ended before it got that far. Verified two ways on 2026-08-20: a workspace forked from the live version reports `notSet` on every tag, and the served `gtm.js` for `GTM-NRQ9HDL9` contains **no `"consent":[…]` array on any tag** in 432 KB of payload. Trigger `19` and the `DLV - method` variable are unpublished for the same reason, so the `contact_click` push currently has no consumer in the live container at all.

This is not "the Ads tags fire straight through an opt-out" — Google's own tags honour Consent Mode natively and downgrade to cookieless pings when `ad_storage` is denied. What is missing is the stronger GTM-level check that blocks the tag outright. **A GTM change only counts once a version is published; an unpublished workspace reads exactly like a finished fix to anyone inspecting the container through the API.** Check `list_versions` and the served `gtm.js`, not a workspace, before recording anything here as done.
